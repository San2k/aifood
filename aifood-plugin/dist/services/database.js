...
export class DatabaseService {
     ...
     */
    async logFood(entry) {
        const result = await this.pool.query(`INSERT INTO food_log_entry (
     ...
     */
    async getEntriesByDate(odentity, date) {
        const startOfDay = new Date(date);
        startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(date);
        endOfDay.setHours(23, 59, 59, 999);
        const result = await this.pool.query(`SELECT * FROM food_log_entry
       WHERE odentity = $1
         AND consumed_at >= $2
         AND consumed_at <= $3
         AND is_deleted = FALSE
       ORDER BY consumed_at ASC`, [odentity, startOfDay, endOfDay]);
        return result.rows.map((row) => this.mapRowToEntry(row));
    }
    /**
     * Get entries for a date range
     */
    async getEntriesByDateRange(odentity, startDate, endDate) {
        const result = await this.pool.query(`SELECT * FROM food_log_entry
       WHERE odentity = $1
         AND consumed_at >= $2
         AND consumed_at <= $3
         AND is_deleted = FALSE
       ORDER BY consumed_at ASC`, [odentity, startDate, endDate]);
        return result.rows.map((row) => this.mapRowToEntry(row));
    }
    /**
     * Calculate daily totals
     */
    async getDailyTotals(odentity, date) {
     ...
     */
    async getWeeklyTotals(odentity, endDate) {
     ...
     */
    async deleteEntry(id, odentity) {
        const result = await this.pool.query(`UPDATE food_log_entry
       SET is_deleted = TRUE
       WHERE id = $1 AND odentity = $2`, [id, odentity]);
        return (result.rowCount ?? 0) > 0;
    }
    /**
     * Get user goals
     */
    async getGoals(odentity) {
        const result = await this.pool.query('SELECT * FROM user_goals WHERE odentity = $1', [odentity]);
    ...
    async setGoals(goals) {
        const result = await this.pool.query(`INSERT INTO user_goals (odentity, target_calories, target_protein, target_carbs, target_fat, target_fiber)
       VALUES ($1, $2, $3, $4, $5, $6)
