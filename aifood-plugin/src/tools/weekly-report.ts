...
export function createWeeklyReportTool(db: DatabaseService) {
  return {
    ...
    handler: async (params: ReportParams, ctx: ToolContext) => {
      ...
      const goalAdherence = goals?.targetCalories
        ? {
            daysOnTarget: daysWithEntries.filter(
              (d) => d.calories >= goals.targetCalories! * 0.9 && d.calories <= goals.targetCalories! * 1.1
            ).length,
